class Solution {
   ...
   public long kMirror(int base, int count) {
       ...
       for (int length = 1; ; ++length) {
           // Find the start and end range based on the half-length to construct palindrome efixes
            ...
            for (int i = start; i < end; i++) {
                ...
                // Construct the second half of the palindrome number based on the length's rity
                for (int j = length % 2 == 0 ? i : i / 10; j > 0; j /= 10) {
                    palindrome = palindrome * 10 + j % 10; // Append digits in reverse order  form the palindrome
                }
                ...
                if (isPalindrome(baseRepresentation.toCharArray())) {
                    ...
                    if (--count == 0) { // Decrement count and check if 'n' k-mirror numbers e found
